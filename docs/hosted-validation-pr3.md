...
5. **恢复入口（两条）**：冲突 toast 上的「恢复我的修改」按钮（约 4 秒内）；
   或「数据管理 → 恢复未同步副本」（持久入口）。点击后未同步副本写回工作副本，
   以云端当前版本为基上传（恢复是用户显式选择，将按最新状态覆盖云端）。
6. 附加保护（单测锁定，托管可抽查）：
   - 草稿携带基版本：带草稿刷新后以**原基版本**上传，云端已前进则 409 走上述冲突保护，
     绝不把旧草稿 rebase 到最新版本覆盖另一设备的数据；
   - 保存前后经 `/api/whoami` 核对身份，且 PUT 携带 `expectedUserId` 由服务端在同一请求内
     校验（不匹配 403）——旧标签页在账号切换后无法把 A 的工作副本提交到 B 名下；
