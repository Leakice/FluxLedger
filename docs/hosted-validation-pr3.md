...
4. 判定：浏览器 2 的 PUT 返回 **409**；随后自动 GET 云端版本，界面出现冲突 toast
   （「同步冲突——已重新加载云端数据，未同步的修改已被云端版本取代」），**工作副本（内存与缓存）
   重载为云端状态**：浏览器 1 的那笔交易出现在浏览器 2 的界面上，浏览器 2 刚才那笔编辑被
   云端版本显式取代（toast 已告知）。云端数据不被静默覆盖，另一设备的数据不消失。
5. 附加保护（单测锁定，托管可抽查）：保存前后都会经 `/api/whoami` 核对身份——旧标签页在
   账号切换后继续编辑时，不会把 A 的工作副本提交到 B 名下（PUT 前中止并发 `session-changed`
   事件整页刷新）；身份无法核实（网络失败重试后仍失败）时拒绝写入（fail closed），草稿保留。
