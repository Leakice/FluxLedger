...
5. **恢复入口（两条）**：冲突 toast 上的「恢复我的修改」按钮（约 4 秒内）；
   ...
   一份起逐份进行。
   - **上限解脱流程（可完成）**：备份满且有未同步草稿时，恢复被阻止属预期；处理路径为
     「数据管理 → 导出数据 → 放弃未同步修改（两步确认）→ 逐份恢复」。保存请求在途时
     放弃会被暂缓（提示稍候重试），防止在途请求把已放弃内容写回云端造成界面与实际
     保存结果不一致；保存完成后若草稿已被同步，放弃不再适用（内容已在云端，可手动
     删除或恢复备份）。放弃只清本页
     草稿（工作副本回到最近一次同步的云端状态），不动备份与云端；恢复出的副本若再遇
     409 会被重新保护入列，再次恢复即以最新云端版本为基同步。**恢复前保护当前未同步副本**：本页有草稿时先把当前工作副本
   追加进备份列表再恢复（列表已满则阻止恢复并提示导出处理，绝不覆盖未备份草稿）；
   每次恢复后其余备份的恢复入口保持可用。恢复以云端当前版本为基上传（恢复是用户显式选择，将按最新状态覆盖云端）。
6. 附加保护（单测锁定，托管可抽查）：
