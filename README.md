...
[在线体验](http://leakice.cn)（需科学上网，配置见[在线访问](#在线访问)） · [界面预览](#界面预览) · [快速开始](#快速开始) · [数据与备份](#数据与备份)
...
还款页按「付款账户 → 信用账户」聚合还款，同一对账户的多笔还款汇总展示。流线等高表示路径，中央直接标出金额，不用粗细比较金额大小；展开可查看关联消费与还款明细。账户余额和未还负债按所选期间截止日计算。
...
![390 像素宽度下的移动端交易记录](docs/images/transactions-mobile.webp)

四张截图均来自同一套默认虚构账本：2026 年 4—9 月，共 66 条记录。默认展示 9 月，金额统一使用人民币（¥）；浅色总览、还款页和手机交易页使用中文；手机图完整展示搜索框及最新三张交易卡片。

## 在线访问

在线入口为 [leakice.cn](http://leakice.cn)。**该站点在国内网络环境下无法直连，访问前需要开启科学上网**，并按以下顺序配置：

1. 将科学上网模式调整为全局。
2. 将 `leakice.cn` 添加到科学上网软件的规则栈中，并确保排在 DIRECT CN 规则**之前**——`.cn` 在大部分机场的规则栈中默认直连，不调整顺序时请求会绕过代理直接超时。

配置后仍无法访问时，可以按[快速开始](#快速开始)在本地运行，数据只存在浏览器本地，与线上站点互不影响。
...
演示数据采用虚构的「日常生活 + 兼职收入」场景，金额、描述与展示卡尾号均为示例。日期固定，方便复现图表和截图；不会连接个人账户或读取真实流水。

| 场景 | 可以观察的功能 |
| --- | --- |
| 4—9 月工资、兼职及日常消费 | 六个月的收支变化、月度对比与交易频次。 |
| 两个储蓄账户、白条与花呗 | 多账户资金分配、分类占比，以及点击桑基图筛选交易。 |
| 房租、超市、聚餐、订阅、出行与购物 | 六种支出分类、描述搜索与账户筛选。 |
| 8 月书桌消费 ¥1,200，9 月全额还清 | 跨月关联还款，消费与还款分别归属各自月份。 |
| 笔记本消费 ¥2,400，由两个账户偿还 ¥600 + ¥400 | 分次还款后剩余 ¥1,400；另有已还清聚餐和未还跑鞋、出游消费。 |
| 白条额度从 8 月 ¥2,000 调整至 9 月 ¥3,000 | 按日期取最新额度，额度不会累加为收入。 |

9 月收入 **¥11,000**、消费支出 **¥10,560**、净收入 **¥440**；还款 **¥2,760** 单独统计，月底未还负债为 **¥2,440**（白条 ¥1,400 + 花呗 ¥1,040）。

当月有效额度为 **¥4,600**，资金容量合计 **¥15,600**。六类消费分别为购物 ¥3,040、生活缴费 ¥3,000、餐饮 ¥1,720、娱乐 ¥1,200、订阅 ¥800、其他 ¥800；未分配容量 **¥5,040（32.3%）**。年度订阅和周末出行让小额分类也有足够的可视空间。还款页的三条路径金额为 **¥1,800 / ¥560 / ¥400**，合计 ¥2,760。

已有浏览器账本优先于 seed，更新代码不会覆盖原有数据。要查看新版演示，可用无痕窗口打开本地应用。四张截图均无需导入额外数据，复现方式如下：

1. 在全新无痕窗口首次打开应用，保留默认的 **2026 年 9 月、全部账户、全部分类**。
2. 桌面视口设为 **1440 × 1100**，选择 **ZH + 日间模式**，截取完整总览页。
3. 切换 **EN + 夜间模式**，截取完整深色总览页；随后切回 **ZH + 日间模式**，进入「还款记录」并截取完整页面。
4. 进入「交易记录」，保留全部记录且搜索框为空，视口设为 **390 × 1046**。滚动至交易搜索框距视口顶部约 16px，截取搜索筛选栏与最新三张完整交易卡片。底部留约 8px 空白，在第四张卡片出现前结束；若系统字体使布局变化，微调视口高度，确保卡片和操作按钮完整。

截图使用正常页面渲染，不隐藏记录或修改显示金额。已有本地数据的普通窗口不会自动变为演示账本。
...
[leakice.cn](http://leakice.cn) 需要科学上网才能访问。请先按[在线访问](#在线访问)一节的步骤确认全局模式已开启、且 `leakice.cn` 规则排在 DIRECT CN 之前；配置后仍无法打开时，可按[快速开始](#快速开始)在本地运行。
