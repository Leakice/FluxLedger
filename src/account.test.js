...
test('v1 accounts and merged navigation survive saves, deletion, imports and reload', async t => {
  ...
  try {
    ...
    assert.deepEqual(app.rows.value.map(row => row.id), [], 'All applies the selected category');
    app.category.value = 'Shopping';
    assert.deepEqual(app.rows.value.map(row => row.id), [999], 'All category filtering still spans dates');
    app.category.value = 'All categories';
