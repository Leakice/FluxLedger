  ...
  <div id="frequency-chart" @mouseleave="active=null" @keydown.esc="active=null">
    <svg viewBox="0 0 360 110" :aria-label="label">
      ...
      <g v-for="(point,i) in points" :key="i" class="frequency-point" :class="{'is-active':active===i}">
        ...
        <rect :x="point.x-24" y="28" width="48" height="80" fill="transparent" tabindex="0" role="img" :aria-label="`${point.label}: ${point.count}`" @mouseenter="active=i" @focus="active=i" @blur="active=null" @click="active=active===i?null:i"/>
      </g>
    </svg>
  </div>
</template>
