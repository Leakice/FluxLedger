...
<template>
  <dialog ref="dialog" class="edit-dialog">
    <form @submit.prevent="save">
      ...
      <div class="form-row"><label>{{ t('Last four digits') }}<input v-model="form.last4" inputmode="numeric" pattern="[0-9]{4}" maxlength="4" required placeholder="4329"></label><label>{{ t('Card network') }}<select v-model="form.network"><option>Visa</option><option>Mastercard</option><option>UnionPay</option><option>Amex</option><option>Other</option></select></label></div>
