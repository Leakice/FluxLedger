...
const accountTypes = ['Savings card', 'Credit card', 'Online banking'];
...
<template>
  <dialog ref="dialog" class="edit-dialog">
    <form @submit.prevent="save">
      ...
      <div class="card-preview" :style="{'--card-color':form.color}"><span>{{ t(form.network) }}</span><strong v-if="!form.noLast4">•••• &nbsp; •••• &nbsp; •••• &nbsp; {{ form.last4||'0000' }}</strong><small>{{ form.name||t('Card name') }}</small><i>◇</i></div>
