...
function open(type, date, entry = null) {
  Object.assign(form, { id: null, description: type === 'credit' ? 'Credit limit' : '', amount: '', type, category: type === 'credit' ? 'Credit limit' : type === 'income' ? 'Salary' : 'Food & Drinks', date, card: props.cards[0]?.id || '' }, entry || {});
...
<template>
  <dialog ref="dialog" class="edit-dialog">
    <form @submit.prevent="save">
      ...
      <label>{{ t('Description') }}<input v-model="form.description" required maxlength="80" :placeholder="t('e.g. Groceries')"></label>
