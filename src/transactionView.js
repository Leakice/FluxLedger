...
// All spans dates and categories while respecting the selected accounts. Typed views
...
export function selectTransactionRows(entries, {
  ...
  return entries.filter(entry => {
    if (cardIds.length && !cardIds.includes(entry.card) && !cardIds.includes(entry.toCard)) return false;
    if (kind !== 'all') {
