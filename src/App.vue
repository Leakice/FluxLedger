...
const entryDescription=entry=>transactionDescription(entry,t);
...
<template>
  ...
  <main>
    ...
    <section v-else-if="page==='Repayment records'" id="repayment-records">
      ...
      <div class="table-wrap" v-if="repaymentRows.length"><table><thead><tr><th>{{ t('Date') }}</th><th>{{ t('Description') }}</th><th>{{ t('Account') }}</th><th>{{ t('Amount') }}</th><th>{{ t('Actions') }}</th></tr></thead><tbody><tr v-for="entry in repaymentRows" :key="entry.id"><td>{{ entry.date }}</td><td>{{ entryDescription(entry) }}</td><td>{{ cardName(entry.card) }} → {{ cardName(entry.toCard) }}</td><td>{{ money(entry.amount) }}</td><td><button @click="openForm('repayment',entry)">{{ t('Edit') }}</button><button @click="remove(entry)">{{ t('Delete') }}</button></td></tr></tbody></table></div>
    </section>
    <section v-else id="transactions">
      ...
      <div class="table-wrap desktop-transactions"><table><thead><tr><th v-for="item in ['Description','Type','Category','Date','Account','Amount','Actions']" :key="item"><select v-if="item==='Account'" class="transaction-column-filter" v-model="accountFilter" :aria-label="t('Filter account')"><option value="">{{ t('All accounts') }}</option><option v-if="accountFilter==='__custom__'" value="__custom__" disabled>{{ t('Custom selection') }} ({{ cards.length }})</option><option v-for="card in bankCards" :key="card.id" :value="card.id">{{ cardName(card.id) }}</option></select><select v-else-if="item==='Category'" class="transaction-column-filter" v-model="category" :aria-label="t('Categories')"><option v-for="item in filterCategories" :key="item" :value="item">{{ t(item) }}</option></select><template v-else>{{ t(item) }}</template></th></tr></thead>
        <TransitionGroup name="row" tag="tbody"><tr v-for="entry in rows" :key="entry.id"><td>{{ entryDescription(entry) }}<small v-if="entry.onCredit&&purchaseFor(entry.id)" class="entry-detail">{{ t(purchaseFor(entry.id).status) }} · {{ t('Amount due') }} {{ money(purchaseFor(entry.id).due) }}</small></td><td><span class="type-badge" :class="entry.type">{{ t(entry.onCredit?'Credit purchase':entryKinds.find(k=>k.type===entry.type)?.label) }}</span></td><td>{{ t(entry.category) }}</td><td>{{ entry.date }}</td><td>{{ cardName(entry.card) }}<template v-if="entry.type==='repayment'"> → {{ cardName(entry.toCard) }}<small class="entry-detail">{{ t('Linked credit purchase') }}: {{ entryDescription(entries.find(p=>p.id===entry.purchaseId)) }}</small></template></td><td :class="entry.type">{{ entry.type==='credit'?'':entry.type==='income'?'+':'−' }}{{ money(entry.amount) }}</td><td><div class="row-actions"><button v-if="entry.onCredit&&purchaseFor(entry.id)?.due>0" class="row-edit" @click="openRepayment(entry)">{{ t('Repay credit') }}</button><button class="row-edit" :aria-label="t('Edit transaction')+' '+entry.description" @click="openForm(entry.type,entry)">✎ {{ t('Edit') }}</button><button class="row-delete" :aria-label="t('Delete transaction')+' '+entry.description" @click="remove(entry)">×</button></div></td></tr></TransitionGroup>
      ...
      <TransitionGroup name="row" tag="ul" class="mobile-transactions" :aria-label="t('Transactions')">
        <li v-for="entry in rows" :key="entry.id" class="transaction-card">
          ...
          <h3>{{ entryDescription(entry) }}</h3><small v-if="entry.onCredit&&purchaseFor(entry.id)" class="entry-detail">{{ t(purchaseFor(entry.id).status) }} · {{ t('Amount due') }} {{ money(purchaseFor(entry.id).due) }}</small>
          <strong class="transaction-amount" :class="entry.type">{{ entry.type==='credit'?'':entry.type==='income'?'+':'−' }}{{ money(entry.amount) }}</strong>
          <dl><div><dt>{{ t('Category') }}</dt><dd>{{ t(entry.category) }}</dd></div><div><dt>{{ t('Account') }}</dt><dd>{{ cardName(entry.card) }}<template v-if="entry.type==='repayment'"> → {{ cardName(entry.toCard) }}<small class="entry-detail">{{ t('Linked credit purchase') }}: {{ entryDescription(entries.find(p=>p.id===entry.purchaseId)) }}</small></template></dd></div></dl>
